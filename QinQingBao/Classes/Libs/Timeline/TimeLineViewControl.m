...
const float CIRCLE_BIG_RADIUS = 4.0;
...
- (void)addProgressBasedOnLabels:(NSArray *)labels currentStatus:(int)currentStatus {
    ...
    for (UILabel *label in labels) {
        ...
        [self configureBezierCircle:circle withCenterY:yCenter idx:i];
...
- (void)configureBezierCircle:(UIBezierPath *)circle withCenterY:(CGFloat)centerY idx:(int)idx{
    if (idx == 0)
    {
