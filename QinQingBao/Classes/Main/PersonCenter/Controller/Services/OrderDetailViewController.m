...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (section == 0) {
        return 1;
    }else if (section == 3){    //工单凭证
        if (!self.workPicInfo || self.workPicInfo.pic_info.count == 0) {
            return 1;
        }
        //return 2;
    }else if (section == 5){   //评论
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        SettlementCell *settlementCell = [SettlementCell createSettlementCellWithTableView:tableView];
        [settlementCell setDataWithOrderModel:self.orderInfor];
        cell = settlementCell;
        ...
        BillCell *billCell = [BillCell createBillCellWithTableView:tableView];
        [billCell setDataWithWorkPicModel:self.workPicInfo];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        static BillCell *billCell = nil;
        if (!billCell) {
            billCell = [BillCell createBillCellWithTableView:tableView];
        }
        [billCell setDataWithWorkPicModel:self.workPicInfo];
        return [billCell getCellHeight];
    }else if (indexPath.section == 4 && indexPath.row == 1){
