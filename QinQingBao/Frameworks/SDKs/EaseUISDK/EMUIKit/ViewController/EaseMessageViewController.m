...
{
    [self showHudInView:self.view hint:NSLocalizedString(@"正在加入...",@"Joining the chatroom")];
    ...
    [[EaseMob sharedInstance].chatManager asyncJoinChatroom:chatroomId completion:^(EMChatroom *chatroom, EMError *error){
        if (weakSelf)
        {
            ...
            {
                [strongSelf showHint:[NSString stringWithFormat:NSLocalizedString(@"加入失败",@"join chatroom \'%@\' failed"), chatroomId]];
            }
            else
            {
...
- (void)chatroom:(EMChatroom *)chatroom occupantDidJoin:(NSString *)username
{
    ...
    [self showHint:[NSString stringWithFormat:NSLocalizedString(@"\'%@\'加入聊天室\'%@\'", @"\'%@\'join chatroom\'%@\'"), username, chatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
}

- (void)chatroom:(EMChatroom *)chatroom occupantDidLeave:(NSString *)username
{
    CGRect frame = self.chatToolbar.frame;
    [self showHint:[NSString stringWithFormat:NSLocalizedString(@"\'%@\'离开聊天室\'%@\'", @"\'%@\'leave chatroom\'%@\'"), username, chatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
}
...
- (void)beKickedOutFromChatroom:(EMChatroom *)chatroom reason:(EMChatroomBeKickedReason)reason
{
    if ([_conversation.chatter isEqualToString:chatroom.chatroomId])
    {
        _isKicked = YES;
        CGRect frame = self.chatToolbar.frame;
        [self showHint:[NSString stringWithFormat:NSLocalizedString(@"被踢出聊天室\'%@\'", @"be removed from chatroom\'%@\'"), chatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
...
- (void)_downloadMessageAttachments:(EMMessage *)message
{
    ...
    void (^completion)(EMMessage *aMessage, EMError *error) = ^(EMMessage *aMessage, EMError *error) {
        ...
        {
            [weakSelf showHint:NSLocalizedString(@"缩略图获取失败!", @"thumbnail for failure!")];
        }
    };
...
- (void)_videoMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if ([localPath length] == 0) {
        [self showHint:NSLocalizedString(@"视频获取失败!", @"video for failure!")];
        return;
    }
    ...
    [self showHudInView:self.view hint:NSLocalizedString(@"正在获取视频...", @"downloading video...")];
    ...
    [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
        ...
        if (!error) {
            ...
            [weakSelf showHint:NSLocalizedString(@"视频获取失败!", @"video for failure!")];
        }
    } onQueue:nil];
}
...
- (void)_imageMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if ([imageBody messageBodyType] == eMessageBodyType_Image) {
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            ...
            [weakSelf showHudInView:weakSelf.view hint:NSLocalizedString(@"正在获取大图...", @"downloading a image...")];
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                [weakSelf hideHud];
                if (!error) {
                    //发送已读回执
                ...
                [weakSelf showHint:NSLocalizedString(@"大图获取失败!", @"image for failure!")];
            } onQueue:nil];
        }else{
            //获取缩略图
...
- (void)_audioMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if (downloadStatus == EMAttachmentDownloading) {
        [self showHint:NSLocalizedString(@"正在下载语音，稍后点击", @"downloading voice, click later")];
        return;
    }
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [weakSelf showHudInView:self.view hint:NSLocalizedString(@"录音时间太短了", @"The recording time is too short")];
...
{
    if ([self.conversation.chatter isEqualToString:message.conversationChatter]) {
        [self showHint:NSLocalizedString(@"有透传消息", @"receive cmd message")];
    }
}
            ...
            if ([object conformsToProtocol:@protocol(IMessageModel)]) {
                ...
                if ([messageId isEqualToString:model.message.messageId]) {
                    ...
                    if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:didFailSendingMessageModel:error:)]) {
                        ...
                        {
                            CGRect frame = self.chatToolbar.frame;
                            [self showHint:NSLocalizedString(@"你的消息包含不当言论", @"Your message contains forbidden words") yOffset:-frame.size.height + 50];
                        }
                    }
